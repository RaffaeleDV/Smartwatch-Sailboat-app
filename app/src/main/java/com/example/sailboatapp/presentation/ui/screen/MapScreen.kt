...
fun getDistanceBetweenPoints(latitude1 : String, longitude1 : String, latitude2 : String, longitude2 : String, unit : String = "miles") : Double {
...
fun getDistanceBetweenPointsMeters(latitude1 : String, longitude1 : String, latitude2 : String, longitude2 : String, unit : String = "kilometers".toString()) : Double {
    return getDistanceBetweenPoints(latitude1, longitude1, latitude2, longitude2, unit = "kilometers") * 1000;
}
...
fun Map(navController: NavHostController){
    ...
    if(!checkLocalConnection()){
        ...
        when(anchorRemoteUiState){
            ...
            is AnchorRemoteUiState.Success -> {
                ...
                anchorRemote = (remoteViewModel.anchorRemoteUiState as AnchorRemoteUiState.Success).anchor
                println(anchorRemote)
                var list = anchorRemote.split(" ")
                println(list.toString())
                println("."+list[2]+".")
                anchorRemoteObj = Anchor(list[0],list[1].filterNot { it.isWhitespace() },list[2],list[3])
            }
        }
    }
    ...
    //Ship Position
    if(nmeaData.value.get("latitude").isNullOrEmpty() && nmeaData.value.get("longitude").isNullOrEmpty()){
        if(nmeaDataRemote.get("latitude").isNullOrEmpty() && nmeaData.value.get("longitude").isNullOrEmpty())
            ship_position
        else
            ship_position = LatLng(nmeaDataRemote.get("latitude")!!.toDouble(),nmeaDataRemote.get("longitude")!!.toDouble())
    }else{
        if(nmeaData.value.get("latitude")!!.equals("0.0")){
            ship_position
        }else{
            ship_position = LatLng(nmeaData.value.get("latitude")!!.toDouble(),nmeaData.value.get("longitude")!!.toDouble())
            cameraPositionState = rememberCameraPositionState {
                position = CameraPosition.fromLatLngZoom(ship_position, 10f)
            }
            println(anchorLocal.latitude +" "+anchorLocal.longitude +"  "+ ship_position.toString())
            println(checkAnchorDistance(LatLng(anchorLocal.latitude.toDouble(),anchorLocal.longitude.toDouble()),ship_position))
            if(checkAnchorDistance(LatLng(anchorLocal.latitude.toDouble(),anchorLocal.longitude.toDouble()),ship_position)){
